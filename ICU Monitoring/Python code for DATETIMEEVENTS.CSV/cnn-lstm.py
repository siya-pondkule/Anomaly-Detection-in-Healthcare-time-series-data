...
print(f"\n=== Processing {DATA_PATH.name} for CNN-LSTM-based Anomaly Detection ===")
...
# ============================================================
# Preprocess (create time difference feature)
# ============================================================
...
df_cnnlstm = df[['time_diff_min']].dropna().reset_index(drop=True)
print(f"🧮 Using feature: {list(df_cnnlstm.columns)}")

# ============================================================
# Scale data and create sequences
# ============================================================
scaler = StandardScaler()
scaled_data = scaler.fit_transform(df_cnnlstm)
...
print(f"📏 CNN-LSTM input shape: {X.shape}")

# ============================================================
# Define CNN-LSTM Autoencoder model
# ============================================================
model = Sequential([
    Conv1D(filters=64, kernel_size=3, activation='relu', padding='same', input_shape=(SEQ_LEN, X.shape[2])),
...
results_file = OUTPUT_DIR / "DATETIMEEVENTS_CNN_LSTM_Results.csv"
...
summary_file = SUMMARY_DIR / "CNN_LSTM_DATETIMEEVENTS_Summary.csv"
...
plt.title("CNN-LSTM Autoencoder Anomaly Detection - DATETIMEEVENTS.csv")
plt.xlabel("Sequence Index")
plt.ylabel("Reconstruction Error (MSE)")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(OUTPUT_DIR / "DATETIMEEVENTS_CNN_LSTM_AnomalyPlot.png")
plt.close()

print(f"📊 Plot saved → {OUTPUT_DIR / 'DATETIMEEVENTS_CNN_LSTM_AnomalyPlot.png'}")
print("\n✅ CNN-LSTM anomaly detection training completed successfully!")
