...
# ======================
# Data Preprocessing and Sequence Creation (FIXED)
# ======================
def preprocess_callouts(df):
    # Convert timestamps - FIX: Changed pd.to_to_datetime to pd.to_datetime
    df['createtime'] = pd.to_datetime(df['createtime'])
    df['outcometime'] = pd.to_datetime(df['outcometime'])
    df['acknowledgetime'] = pd.to_datetime(df['acknowledgetime'])
    
    # Feature Engineering: Time differences in hours
    ...
    # Handle NaN values in engineered features (using mean for imputation)
    df['ack_lag_hours'].fillna(df['ack_lag_hours'].mean(), inplace=True)
    
    # Select Features: Numerical flags, Engineered features, and Categorical columns
    features_df = df[[
...
if __name__ == "__main__":
    ...
    print("WARNING: Dataset size is only 77 records. Deep model results may not generalize well.")
    ...
    history = lstm_autoencoder.fit(X_seq, X_seq, epochs=50, batch_size=4, validation_split=0.1, verbose=0)
    ...
    # Plot anomalies (Focus on duration_hours)
    plt.figure(figsize=(12,6))
    
    duration_data = X_processed['duration_hours'].iloc[SEQUENCE_LENGTH-1:]
    plot_indices = X_processed.index[SEQUENCE_LENGTH-1:]
    
    plt.scatter(plot_indices, duration_data, 
                c=mse_per_sequence, cmap='viridis', s=20, label='Duration (Color by Error)')
    
    y_pred_model = (mse_per_sequence >= eval_results['Threshold']).astype(int)
    model_anomalies_indices = plot_indices[y_pred_model == 1]
    
    plt.scatter(model_anomalies_indices, X_processed.loc[model_anomalies_indices, 'duration_hours'], 
