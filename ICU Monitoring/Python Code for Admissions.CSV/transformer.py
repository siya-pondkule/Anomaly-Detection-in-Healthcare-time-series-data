...
def build_transformer_autoencoder(seq_len, n_features):
    ...
    # Positional Embedding (Crucial for Transformer on time series)
    positions = tf.range(start=0, limit=seq_len, delta=1)
    # The Embedding layer requires a max token length
    pos_emb = layers.Embedding(input_dim=seq_len, output_dim=n_features)(positions)
    x = inputs + pos_emb # Add positional info to input

    # Encoder Block
    x = transformer_encoder(x, head_size, num_heads, ff_dim, dropout)
    
    # Bottleneck (Flatten and Dense)
    x = layers.GlobalAveragePooling1D(data_format="channels_first")(x)
    encoded = layers.Dense(8, activation='relu', kernel_regularizer=regularizers.l1(1e-4))(x)

    # Decoder (Reconstruction via Dense + Reshape) - Simplification
    # Complex decoding structure is avoided for simplicity and convergence speed
...
if __name__ == "__main__":
    ...
    history = transformer_autoencoder.fit(X_seq, X_seq, epochs=50, batch_size=32, validation_split=0.1, verbose=0)
