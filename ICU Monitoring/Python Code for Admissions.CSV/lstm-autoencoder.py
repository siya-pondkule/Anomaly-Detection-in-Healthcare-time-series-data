...
if __name__ == "__main__":
    ...
    gt_array = gt_array_full[SEQUENCE_LENGTH - 1:] # Ground truth for the last item in sequence

    # Train Model
    lstm_autoencoder = build_lstm_autoencoder(SEQUENCE_LENGTH, n_features)
    history = lstm_autoencoder.fit(X_seq, X_seq, epochs=50, batch_size=32, validation_split=0.1, verbose=0)
    ...
    plt.scatter(X_processed.index[SEQUENCE_LENGTH-1:], X_processed['LOS_hours'].iloc[SEQUENCE_LENGTH-1:], 
                c=mse_per_sequence, cmap='viridis', s=20, label='LOS (Color by Error)')
    ...
    plt.scatter(model_anomalies_indices, X_processed.loc[model_anomalies_indices, 'LOS_hours'], 
                color="red", marker="o", edgecolors='red', facecolors='none', s=100, label=f'Predicted Anomalies ({len(model_anomalies_indices)})')
